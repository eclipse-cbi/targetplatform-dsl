...
public class LocationIndexBuilder {
  ...
  public ListMultimap<String, Location> getLocationIndex(final TargetPlatform targetPlatform) {
    ...
    final Function<Location, String> _function = (Location it) -> {
      return it.getUri();
    };
  ...
  private List<Location> getLocations(final Set<TargetPlatform> visited, final List<TargetPlatform> toBeVisited) {
    ...
    final Consumer<TargetPlatform> _function = (TargetPlatform it) -> {
      final LinkedList<IncludeDeclaration> includes = CollectionLiterals.<IncludeDeclaration>newLinkedList();
      final Consumer<TargetContent> _function_1 = (TargetContent content) -> {
        if ((content instanceof Location)) {
          boolean _isEmpty = includes.isEmpty();
          boolean _not = (!_isEmpty);
          if (_not) {
            locations.addAll(this.getLocationFromVisitedIncludes(it, includes, visited));
            includes.clear();
          }
      ...
      ListExtensions.<TargetContent>reverseView(it.getContents()).forEach(_function_1);
      boolean _isEmpty = includes.isEmpty();
      boolean _not = (!_isEmpty);
      if (_not) {
        locations.addAll(this.getLocationFromVisitedIncludes(it, includes, visited));
  ...
  private List<Location> getLocationFromVisitedIncludes(final TargetPlatform parent, final List<IncludeDeclaration> includes, final Set<TargetPlatform> visited) {
    final Function1<IncludeDeclaration, TargetPlatform> _function = (IncludeDeclaration it) -> {
      return this.getImportedTargetPlatform(parent.eResource(), it);
    };
    final Function1<TargetPlatform, Boolean> _function_1 = (TargetPlatform it) -> {
    ...
    while ((!queue.isEmpty())) {
      {
        ...
        final Function1<IncludeDeclaration, TargetPlatform> _function = (IncludeDeclaration it) -> {
          return this.getImportedTargetPlatform(t.eResource(), it);
        };
  ...
  private boolean checkIncludeCycle(final TargetPlatform targetPlatform, final Set<TargetPlatform> visited, final LinkedList<TargetPlatform> s) {
    ...
    final Function1<IncludeDeclaration, TargetPlatform> _function = (IncludeDeclaration it) -> {
      return this.getImportedTargetPlatform(context, it);
    };
